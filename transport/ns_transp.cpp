...
// flow control
{
    ...
    eps= P.get<double>("SurfTension.JumpWidth");    lambda= P.get<double>("SurfTension.RelPos");    sigma_dirt_fac= P.get<double>("SurfTension.DirtFactor");
    instat_scalar_fun_ptr sigmap  = 0;
    if (P.get("SurfTens.VarTension", 0))
    {
        sigmap  = &sigma_step;
    }
    else
...
//             new BlockMatrixSolverCL<GCRSolverCL<StokesSolverAsPreCL> > (gcr);
    ...
    if (P.get("NavierStokes.Nonlinear", 0.0)==0.0)
        ...
        navstokessolver = new AdaptFixedPtDefectCorrCL<InstatNavierStokes2PhaseP2P1CL>(Stokes, *stokessolver, P.get<int>("NavierStokes.Iter"), P.get<double>("NavierStokes.Tol"), P.get<double>("NavierStokes.Reduction"));
    // Level-Set-Solver
#ifndef _PAR
    SSORPcCL ssorpc;
...
//         stokessolverfactory.GetPVel()->GetFinest(), stokessolverfactory.GetPPr()->GetFinest());
    ...
    Ensight6OutCL ensight( P.get<std::string>("Ensight.EnsCase") + ".case", (P.get("Ensight.EnsightOut", 0) ? P.get<int>("Time.NumSteps")/P.get<int>("Ensight.EnsightOut")+1 : 0), P.get<int>("Ensight.Binary"), P.get<int>("MasterOut"));
...
//         if (P.get("Transport.DoTransp", 0)) massTransp.DoStep( step*P.get<double>("Time.StepSize"));
        if (P.get<int>("SurfTransp.DoTransp")) {
        ...
        // grid modification
        bool doGridMod= P.get("AdaptRef.Freq", 0) && step%P.get("AdaptRef.Freq", 0) == 0;
        if (doGridMod) {
...
//        P1XtoP1 (*massTransp.c.RowIdx, massTransp.c.Data, p1idx, c_out.Data, c_in.Data, lset.Phi, MG);         
        ...
        bool ensightoutnow = P.get<int>("Ensight.EnsightOut") && step%P.get<int>("Ensight.EnsightOut")==0;
        bool vtkoutnow = P.get("VTK.VTKOut", 0) && step%P.get("VTK.VTKOut", 0)==0;
        
        //if (ensightoutnow || P.get("VTK.VTKOut", 0) && vtkoutnow){ solution on part stuff }
...
int main (int argc, char** argv)
{
  ...
  try
  {
    ...
    double tmp = P.get<double>("Exp.RadInlet");
    DROPS::BuildDomain( mg, P.get<std::string>("DomainCond.MeshFile"), P.get<int>("DomainCond.GeomType"), P.get<std::string>("Restart.Inputfile"), tmp);
    P.put("Exp.RadInlet", tmp);
    ...
    DROPS::AdapTriangCL adap( *mg, P.get<double>("AdaptRef.Width"), P.get<int>("AdaptRef.CoarsestLevel"), P.get<int>("AdaptRef.FinestLevel"), ((P.get("Restart.Inputfile", std::string("none")) == "none") ? P.get<int>("AdaptRef.LoadBalStrategy") : -P.get<int>("AdaptRef.LoadBalStrategy")), P.get<int>("AdaptRef.Partitioner"));
