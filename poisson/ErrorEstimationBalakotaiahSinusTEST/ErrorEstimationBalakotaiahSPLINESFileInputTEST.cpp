...
double ErrorBalakotaiahHQSplines(double X, double Y, double T, int XSTEPS, double INCX, double INCT, double RE, double WE, double COTBETA){
    ...
    double dt=1.1e-5; //time-increment for temopral differentiation. Do not mix it up with the time-increment inct_spline of the 
                     //discreteraw data
    //HQUV::HQUV(double X, double T, int XSTEPS, double INCX, double INCT, double RE, double WE, double COTBETA)
...
int main() {   
    ...
    double errorBala;
    std::string error_filename = "errorsplineBala.txt";
    std::ofstream errorfile;
    errorfile.precision(16);
    errorfile.open(error_filename.c_str());
    
    //C.f. ErrorEsimationBalakotaiahTESTSPLINESFileInput.cpp:
    ...
    //double t1=4.e-3;
    //double t2=t1+1.1e-5; 
    //Use the same data and compare the error-results of the present code 
    //with the results from ErrorEsimationBalakotaiahTESTSPLINESFileInput.cpp .
    //Please mind: delta_t in the latter code equals inct_spline of the present 
    //code (time-increment of raw data) and 1.1*delta_t is used in 
    //ErrorEsimationBalakotaiahTESTSPLINESFileInput.cpp as time-increment for
    // temporal differentiation (in the present code tfis increment is denoted with dt).
    ...
    CreateRawData(10000, 1.e-5, 1000, 0.000209);
